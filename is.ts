...
export const Is = {
  ...
  notEmpty(val: any): boolean { return (Array.isArray(val) && !!val.length) || (typeof val === 'string' && !!val.trim.length) || (Types.isObject(val) && !!Object.keys(val).length); },
  ...
  date(val: any): boolean {
    ...
    if (Types.isString(val) || Types.isNumber(val)) {
      let d = new Date(val);
